...
function money($value) {
    return number_format($value)."<sup>đ</sup>";
}
function str_limit($str, $num = 100) {
    $tmp = $num;
    $str = strip_tags($str);
    if (strlen($str) <= $num) {
        return $str;
...
function str_slug($slug) {
    ...
    $slug = str_replace(array('-8220-', '-8221-', '-7776-'), '-', $slug);
    //$slug = preg_replace( '/[^a-zA-Z0-9\-]+/e', '', $slug );
    $slug = str_replace(array('dAg', 'DAg', 'uA', 'iA', 'yA', 'dA', '--', '-8230'), array('dong', 'Dong', 'uon', 'ien', 'yen', 'don', '-', ''), $slug);
